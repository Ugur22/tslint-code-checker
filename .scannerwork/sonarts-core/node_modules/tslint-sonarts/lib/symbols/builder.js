...
class SymbolTableBuilder extends tslint.SyntaxWalker {
    constructor(program) {
        super();
    ...
    visitBinaryExpression(node) {
        if (navigation_1.isAssignment(node)) {
    ...
    registerWriteUsageForAssignment(node) {
        if (node.kind === ts.SyntaxKind.Identifier) {
            this.registerUsageIfMissing(node, table_1.UsageFlag.WRITE);
        }
        ...
        else if (navigation_1.is(node, ts.SyntaxKind.PropertyAssignment)) {
            this.registerWriteUsageForAssignment(node.initializer);
        }
        else if (navigation_1.is(node, ts.SyntaxKind.ShorthandPropertyAssignment)) {
            this.registerWriteUsageForAssignment(node.name);
        }
        else if (navigation_1.is(node, ts.SyntaxKind.SpreadAssignment, ts.SyntaxKind.SpreadElement)) {
            this.registerWriteUsageForAssignment(node.expression);
        }
        else if (navigation_1.is(node, ts.SyntaxKind.BinaryExpression)) {
            this.registerWriteUsageForAssignment(node.left);
        }
    }
    ...
    addVariable(node) {
        ...
        if (declarationName.kind === ts.SyntaxKind.Identifier) {
            ...
            if (node.initializer ||
                navigation_1.is(node, ts.SyntaxKind.Parameter) ||
                (node.parent && navigation_1.is(node.parent, ts.SyntaxKind.ObjectBindingPattern, ts.SyntaxKind.ArrayBindingPattern)))
