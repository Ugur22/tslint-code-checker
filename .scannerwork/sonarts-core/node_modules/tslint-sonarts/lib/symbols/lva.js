...
class LiveVariableAnalyzer {
    ...
    computeSymbolsWithAvailableReads(block) {
        ...
        function usageNode(node) {
            if (navigation_1.isAssignment(node)) {
                const identifier = navigation_1.getIdentifier(node.left);
                if (identifier)
                    return identifier;
            }
            return node;
    ...
    isUsedInNestedFunctions(symbol) {
        return this.symbols
            .allUsages(symbol)
            .some(usage => navigation_1.firstLocalAncestor(usage.node, ...navigation_1.FUNCTION_LIKE) !== this.root);
    }
